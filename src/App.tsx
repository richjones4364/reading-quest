...
import { readingSections } from './data/sampleText';
...
function App() {
  ...
  const handleAnswer = (questionId: number, selectedAnswer: number) => {
    ...
    const allAnswered = Object.keys(answeredQuestions).length === currentSection.questions.length - 1;
    if (allAnswered) {
      const totalCorrect = Object.values(isCorrect).filter(Boolean).length + (correct ? 1 : 0);
      
      // Show encouragement message
      setEncouragementMessage(getRandomEncouragement(totalCorrect, currentSection.questions.length));
